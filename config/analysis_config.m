...
%   References: docs/ARCHITECTURE.md §2 Stage 5, §3.3-3.5, §4.3.
    ...
    a.opticDisc = struct( ...
        ...
        'scoringWeights', struct( ...        % candidate ranking weights (brightness/temporal/size/circularity)
            'brightness', 0.4, ...
            'temporal',   0.3, ...
            'size',       0.2, ...
            'circularity', 0.1), ...
    ...
    % A single detected optic disc (the primary structural landmark) must
    % already yield at least 'medium' advisory confidence, so its weight is at
    % or above the 'medium' threshold (test_buildEvidenceConfidenceLevels).
    a.confidence = struct( ...
